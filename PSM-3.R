...
mod_test1<- glm(Hdtype~Hb+PVC+DC3+DF3+BPR3+UFR3+AP3+VP3, 
                data,
                family="binomial")

mod_test2<- lm(X4Hr ~  Hb+PVC+DC3+DF3+BPR3+UFR3+AP3+VP3, data )
...
data_isf3<-na.omit(data_isf[,c("Hdtype","Hb","PVC","DC3","DF3","BPR3","UFR3","AP3","VP3","X3Hr")])
for(i in 2:9)
  data_isf3[,i]=as.numeric(data_isf3[,i])

psa_n<-matchit(Hdtype~Hb+PVC+DC3+DF3+BPR3+UFR3+AP3+VP3, 
               data=data_isf3,
               distance="glm",
               method="nearest",
               m.order = "largest",
...
psa_o<-matchit(Hdtype~Hb+PVC+DC3+DF3+BPR3+UFR3+AP3+VP3, 
               data = data_isf3,
               distance="glm",
               method="optimal",
               ratio=2)
...
psa_f<-matchit(Hdtype~Hb+PVC+DC3+DF3+BPR3+UFR3+AP3+VP3,
               data = data_isf3,
               distance="glm",
               method="full",
               estimand = "ATE")
...
psa_f2<-matchit(Hdtype~Hb+PVC+DC3+DF3+BPR3+UFR3+AP3+VP3+I(AP3^2)+I(VP3^2),
                data = data_isf3,
                distance="glm",
                method="full",
                estimand = "ATE")
...
mod1<-multinom(X3Hr~Hdtype+DC3+DF3+BPR3+UFR3+AP3+VP3+I(AP3^2)+I(VP3^2),
               data = psa_f2_dat,
               weights=weights)
...
mod0<-multinom(X3Hr~Hdtype+DC3+DF3+BPR3+UFR3+AP3+VP3+I(AP3^2)+I(VP3^2),
               data = data_isf3)
summary(mod0)
